...
model{
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {
      ...
      choice[i,t] ~ bernoulli_logit( mu_c * (ev_later - ev_sooner) );
    }
  }
}
  ...
  { // Local section
    for (i in 1:N) {
      ...
      for (t in 1:(Tsubj[i])) {
        ev_later   = pow(amount_later[i,t], a[i])  / ( 1 + k[i] * delay_later[i,t] );
        ev_sooner  = pow(amount_sooner[i,t], a[i]) / ( 1 + k[i] * delay_sooner[i,t] );
        log_lik[i] = log_lik[i] + bernoulli_logit_lpmf(choice[i,t] | mu_c * (ev_later - ev_sooner) );
      }
    }
  }
}
